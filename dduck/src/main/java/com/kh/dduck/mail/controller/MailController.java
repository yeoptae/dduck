package com.kh.dduck.mail.controller;
...
public class MailController {
   ...
   public String email() {
      // 회원가입 클릭시 lgoin경로에 email.jsp view를 열어준다.
      return "login/email";
   }
...
//----------------------------------------------------------------------------------------------   
   ...
   // 이메일 인증 페이지 맵핑 메소드
      @RequestMapping("/pwSearch/pwSearch.do")
      public String email2() {
         // 회원가입 클릭시 login경로에 pwSearch.jsp view를 열어준다.
   ...
   // mailSending 코드
   @RequestMapping(value = "/pwSearch/pwSearch2.do", method = RequestMethod.POST)
...
//      
      ...
      ModelAndView mv = new ModelAndView(); // ModelAndView로 보낼 페이지를 지정하고, 보낼 값을 지정한다.
      mv.setViewName("/login/pwSearchEnroll"); // 뷰의경로 및 이름
      mv.addObject("dice", dice);
...
//    내가 입력한 인증번호와 메일로 입력한 인증번호가 맞는지 확인해서 맞으면 회원가입 페이지로 넘어가고,
//    틀리면 다시 원래 페이지로 돌아오는 메소드
   @RequestMapping(value = "/member/join_injeung2.do{dice}", method = RequestMethod.POST)
   public ModelAndView join_injeung2(String email_injeung2, @PathVariable String dice,
         HttpServletResponse response_equals) throws IOException {

      System.out.println("마지막 : email_injeung : " + email_injeung2);
      ...
      mv.addObject("e_mail", email_injeung2);

      if (email_injeung2.equals(dice)) {
      ...
      } else if (email_injeung2 != dice) {

         ModelAndView mv2 = new ModelAndView();

         mv2.setViewName("login/list");
